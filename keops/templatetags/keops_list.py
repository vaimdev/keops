...
def result_headers(cl):
    ...
    for i, field_name in enumerate(cl.list_display):
        ...
        content = mark_safe('<span ng-bind-html="item.%s"></span>' % field_name)
        if attr:
            ...
            if field_name == 'action_checkbox':
                yield {
                    "content": mark_safe('<label class="checkbox"><input type="checkbox" class="action-select" ng-click="selectItem()"><i></i></label>'),
                    "text": mark_safe('<label class="checkbox" ng-click="toggleCheckAll()">%s<i></i></label>' % text),
        ...
        th_classes = ['sorting', 'column-{0}'.format(field_name)]
        order_type = ''
        new_order_type = 'asc'
        sort_priority = 0
        sorted = False
...
@register.inclusion_tag("keops/change_list_results.html")
def result_list(cl):
    """
